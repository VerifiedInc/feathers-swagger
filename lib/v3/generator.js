...
function addDefinitionToSchemas (
  schemas,
  definition,
  model,
  modelName,
...
function jsonSchemaRef (ref) {
  if (typeof ref === 'object' && ref.refs) {
    ...
    return {
      'application/json': {
        schema: {
          [type]: refs.map((innerRef) => ({
            $ref: `#/components/schemas/${innerRef}`
          })),
          ...rest
        }
      }
...
class OpenApiV3Generator extends AbstractApiGenerator {
  ...
  getOperationsRefs (service, model, schemaNames) {
    ...
    const refs = {
      findResponse: isPaginationEnabled(service)
        ? schemaNames.pagination(model)
        : modelList,
      getResponse: model,
      createRequest: model,
    ...
    if (typeof this.config.defaults.getOperationsRefs === 'function') {
      Object.assign(
        refs,
        this.config.defaults.getOperationsRefs(model, service)
      );
    }
    ...
    return {
      find ({ tags, security, securities, refs }) {
        ...
        if (!_.isEmpty(refs.queryParameters)) {
          const queryParameters = filterParameter(refs, 'queryParameters', {
          ...
          parameters.push({
            ...
            schema: {
              ...(refs.sortParameter
                ? { $ref: `#/components/schemas/${refs.sortParameter}` }
                : { type: 'object' })
            }
          });
      ...
      create ({ tags, security, securities, refs, multiOperations }) {
        ...
        return {
          ...
          requestBody: {
            required: true,
            content: multi
              ? jsonSchemaRef(refs.createMultiRequest)
              : jsonSchemaRef(refs.createRequest)
          },
          responses: {
            201: {
              description: 'created',
              content: multi
                ? jsonSchemaRef(refs.createMultiResponse)
                : jsonSchemaRef(refs.createResponse)
            },
            401: {
              description: 'not authenticated'
            },
      ...
      custom (
        { tags, modelName, idName, idType, security, securities, refs },
        { method, httpMethod, withId }
      ) {
        const customDoc = {
          tags,
        ...
        if (withId) {
          customDoc.parameters = idPathParameters(
            idName,
            idType,
            `ID of ${modelName}`
          );
  ...
  applyDefinitionsToSpecs (service, model, modelName, refs, schemaNames) {
    if (typeof service.docs.definition !== 'undefined') {
      addDefinitionToSchemas(
        this.specs.components.schemas,
        service.docs.definition,
        model,
        modelName,
        schemaNames
      );
    ...
    if (typeof this.config.defaults.schemasGenerator === 'function') {
      this.specs.components.schemas = Object.assign(
        this.specs.components.schemas,
        this.config.defaults.schemasGenerator(
          service,
          model,
          modelName,
          this.specs.components.schemas
