...
exports.operation = function operation (
  ...
  service,
  defaults = {},
  specDefaults = v2OperationDefaults
) {
  const operation = {};
...
exports.idPathParameters = function idPathParameters (idName, idSeparator) {
  return `{${
    Array.isArray(idName) ? idName.join(`}${idSeparator}{`) : idName
  }}`;
};
...
exports.defaultTransformSchema = function defaultTransformSchema (schema) {
  ...
  if (allowedProperties.anyOf) {
    allowedProperties.anyOf = allowedProperties.anyOf.map(
      defaultTransformSchema
    );
  }

  if (allowedProperties.allOf) {
    allowedProperties.allOf = allowedProperties.allOf.map(
      defaultTransformSchema
    );
  }

  if (allowedProperties.properties) {
    allowedProperties.properties = Object.entries(
...
function determineSchemaPrefix (schemas) {
  const dataSchemaKey = Object.keys(schemas).find((value) =>
    value.endsWith('DataSchema')
  );
  if (dataSchemaKey) {
    return dataSchemaKey.replace(/DataSchema$/, '');
  }
  const querySchemaKey = Object.keys(schemas).find((value) =>
    value.endsWith('QuerySchema')
  );
  if (querySchemaKey) {
    return querySchemaKey.replace(/QuerySchema$/, '');
  }
  const patchSchemaKey = Object.keys(schemas).find((value) =>
    value.endsWith('PatchSchema')
  );
  if (patchSchemaKey) {
    return patchSchemaKey.replace(/PatchSchema$/, '');
  }
  const schemaKey = Object.keys(schemas).find((value) =>
    value.endsWith('Schema')
  );
  if (schemaKey) {
...
exports.createSwaggerServiceOptions = function createSwaggerServiceOptions ({
  schemas,
  docs,
  transformSchema
}) {
  ...
  if (prefix) {
    ...
    if (schemas[resultSchemaKey]) {
      ...
      serviceDocs.schemas[listSchemaName] = {
        type: 'array',
