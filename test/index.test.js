...
describe("feathers-swagger", () => {
  Object.entries({
    ...
    describe(`should serve openapi json file with ${type}`, () => {
      ...
      const startApp = (swaggerConfig) => {
        ...
        return startFeathersApp(app, 6776).then((res) => {
          appTeardown = res;
        });
      };
      ...
      it("default as /swagger.json", async () => {
        ...
        expect(
          (await axios.get("http://localhost:6776/swagger.json")).data
        ).to.deep.equal(app.docs);
      });
      ...
      it("use `docsJsonPath` as /docs.json", async () => {
        ...
        expect(
          (await axios.get("http://localhost:6776/docs.json")).data
        ).to.deep.equal(app.docs);
        await axios
          .get("http://localhost:6776/swagger.json")
          .catch((error) =>
            expect(error.response).to.have.property("status", 404)
          );
      });
    });
  });
  ...
  describe("basic functionality with openapi v2", () => {
    ...
    it("supports id types in config", () => {
      const messagesIdParam =
        app.docs.paths["/messages/{id}"].get.parameters[0];
  ...
  describe("basic functionality with openapi v3", () => {
    ...
    it("serves json specification at docJsonPath", () => {
      expect(app.docs.openapi).to.equal("3.1.0");
      expect(app.docs.info.title).to.equal("A test");
      expect(app.docs.info.description).to.equal("A description");
      expect(app.docs.paths["/messages"]).to.exist;
    });
  ...
  it("fails with invalid openApiVersion", () => {
    const app = feathers();
    expect(() => app.configure(swagger({ openApiVersion: 1 }))).to.throw(
      Error,
      "Unsupported openApiVersion 1! Allowed: 2, 3"
    );
  });
  ...
  describe("appProperty", () => {
    ...
    it("should be docs by default", () => {
      const app = feathers().configure(swagger({ specs }));
    ...
    it("can be specified", () => {
      const app = feathers().configure(
        swagger({ specs, appProperty: "swaggerSpecs" })
      );
    ...
    it("can be disabled", () => {
      const app = feathers().configure(swagger({ specs, appProperty: "" }));
